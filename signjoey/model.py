...
class SignModel(nn.Module):
    ...
    def forward(
        ...
        """
        # ToDo : small processing for pose data
        # integration in the encoder
    ...
    def encode(
        self, sgn: Tensor, sgn_mask: Tensor, sgn_length: Tensor, pose: Tensor, pose_mask: Tensor, pose_length: Tensor
    ) -> (Tensor, Tensor):
        """
        ...
        return self.encoder(
            ...
            src_length=sgn_length, mask=sgn_mask,
            embed_pose= self.pose_embed(x=pose, mask=pose_mask),
            pose_length=pose_length, pose_mask=pose_mask
...
def build_model(
    ...
    pose_embed: SpatialEmbeddings = SpatialEmbeddings(
        **cfg["encoder"]["embeddings"],
        num_heads=cfg["encoder"]["num_heads"],
        input_size=pose_dim,
    )
